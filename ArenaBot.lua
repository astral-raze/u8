script_name("ArenaBot") --54
script_version("1.6") --122
...
local msg = function(text)
    sampAddChatMessage(u8:decode('[AUTOUPDATE] {fff0f5}'..text), 0xFFCD5C5C)
    end
...
function main()
    ...
    if not doesFileExist(getWorkingDirectory()..'/resource/fonts/fontawesome-webfont.tt') then 
        ...
        local json = getWorkingDirectory() .. '\\'..thisScript().name..'-version.json'
        if doesFileExist(json) then os.remove(json) end
        downloadUrlToFile('https://raw.githubusercontent.com/astral-raze//main/update.json', json,
          function(id, status, p1, p2)
            if status == dlstatus.STATUSEX_ENDDOWNLOAD then
              if doesFileExist(json) then
                local f = io.open(json, 'r')
        ...
        if cfg.config.AutoUpdate == 1 then
            autoupdate("https://raw.githubusercontent.com/astral-raze//main/update.json", '['..string.upper(thisScript().name)..']: ', "https://www.blast.hk/threads/138165/")
